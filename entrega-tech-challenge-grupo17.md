...
3. **Deploy em Ambiente de Produção (Central de Experimentos MLflow na AWS):**
   ...
   * *Nota 3 (Arquitetura de Contêineres - Separação de Conceitos):* Para fins de avaliação, destacamos a separação de responsabilidades na arquitetura de contêineres: o servidor do MLflow provisionado na EC2 consome a imagem oficial do MLflow (`ghcr.io/mlflow/mlflow`), enquanto a esteira de CI/CD vinculada ao Docker Hub (`docker-publish.yml`) constrói, testa e versiona o contêiner personalizado do nosso sistema de recomendação (responsável por rodar o pipeline do recomendador).
